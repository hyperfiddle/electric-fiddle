(ns ui-demo.form
  (:require [hyperfiddle.electric3 :as e]
            ...
            [hyperfiddle.electric-forms0 :as forms]))
       ...
       (dom/dd
         (let [[~'_ ~'_ validation-message# :as edit#]
               (e/call ~Control! ~name ~value (-> (assoc ~props :id ~control-id, :aria-errormessage ~validation-message-id)
                                                (dissoc :label)
                                                (clojure.set/rename-keys {:option-label :label})))]
           (dom/p (dom/props {:id ~validation-message-id, :data-role "errormessage"}) ; no native errormessage role
  ...
  (e/client
    ...
    (let [[staff _] r/route]
      (Form!
          (dom/dl
            (e/amb
              (field Input! ::staff staff :disabled true,:Validate (e/fn [staff] (when-not (= "12345" staff) "Wrong staff")),  :required true, :label "staff") 
              (field Typeahead! ::venue nil :Options Venue-picklist :option-label :venue/name, :required true, :label "venue")
              ...
              (field Checkbox! ::penalize false :label "penalize?", :option-label (constantly "Penalize the staff for any last-minute cancellations"))
        ...
        :commit (fn [{::keys [venue block-reason block-mode penalize] :as state}]
                  [`Block-staff-from-venue staff (:venue/id venue) (:db/ident block-reason) block-mode penalize]) ; just a command, no guess
        )
      )))