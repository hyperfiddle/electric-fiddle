...
Perf notes:
* `clojure.datafy` is called on each visible row as you scroll, which implies IO (i.e. git queries over filesystem), so scroll latency is worse on the git demo only afaict. Still, the demo is cool so I'm just gonna leave it.
* also I'm not moving this datafy IO to a worker thread (which I should be), so under load this essay might fall over - LMK if it does. Moving IO to a thread is a one-liner in Electric but we're currently reworking the operator, so it's temporarily disabled.
...
* refactoring my custom electric markdown extensions that power these essays, i.e. dealing with my own busted/experimental designs (I have so many of them because with Electric they are so cheap to make!)
* getting the .git folder baked into the prod image for this demo (debugging and bypassing a docker security check)
* mobile-friendly CSS layout for all the grids 
* **writing this actual essay content** - by far the most time consuming part
...
* encoding hyperlinks as partially applied methods/functions (!) and rendering them appropriately
* object target picker - giving you multiple datafy instances to play with
* plopping in the `datafy-git` POC i had lying around
...
This blog series has a time axis. Part 1 was published on Tuesday the 7th, and today I publish part 2 on Friday the 10th, three days later. By the way, I did not work on this full time, I am currently working on three separate deals and have been spending my mornings writing memos to attorneys and generally feeling very stressed. This is the fun thing in my day, that I look forward to after the real work is done! And Electric Clojure has brought it within my reach of my screw around time to run little experiments like this, cheaply enough to fit in the cracks of my week.
