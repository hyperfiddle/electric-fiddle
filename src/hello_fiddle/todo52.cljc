;; Similarities with Dustin's UI5
;; - optimistic updates
;; - controls don't backpressure users
;; - txs are independent
;; - based on [x dx] and tempid genesis
;; - 4 colors depends on tx-report and presence of dxs

;; Differences with Dustin's UI5:
;; - This is a bottom up approach (start with 4 colored button, then separate layers)
;; - Dustin's UI5 has a single tx-report atom, controls (e.g. Field) self-unregisters when they see their own tx accepted
;;   - In this experiment:
;;     - controls do have a local lifecycle (busy/not busy) and do already monitor the tx status.
;;     - BUT controls are more passive, they don't retract their own dx signal on accepted tx (they should)
;;     - N parallel txs per entity, one tx-report per entity
;;       - no clear pro compared to Dustin's version
;; - In Dustin's UI5, dx are flows, bubbling up through e/amb. Multiplicity is handled by e/amb.
;;   - in this experiment dxs are parallel (hashmap) and reach the transacting logic through a side channel
;; - Dustin's UI5 do not have any txid (only use case here it to differentiate parallel txs, would be handled by e/amb, to be removed)

;; Missing bits (no risk, already solved)
;; - controls rejects external updates until user tx is accepted (don't damage user input, also while focused)
;; - popovers to accumulate and commit/rollback txs at various level (e.g. Field, Form, Row)

;; Essential learnings
;; [x dx] looks like a good candidate to be modeled as an (Incseq (Incseq x) (Incseq dx))
;; The index looks like a good candidate to be modeled as a Spine - if needed at all
;; Can't model instant optimistic updates over infinite sequences without V3 dynamic siting.
;;   can't transfer infinite sequence over the wire
;;   can't emit effects on the remote peer without latency
