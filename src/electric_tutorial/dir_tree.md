...
* Values transfer over network only when platform interop requires it
* Platform interop = foreign Clojure/Script code
* the java file handle `h` is on the server because `java.nio.file.Path/of`, `clojure.java.io/file` etc are on the server. Note, as it is a reference type, `h` is not serializable, it cannot cross to the client even if you wanted it to.
* `(e/server (.getName h))` passes `h` to Clojure interop `.getName`, which forces `h` to be on the server, which it already is because it came from `(e/server ... (java.nio.file.Path/of ...))`
...
* (This model is not request/response; that would be too slow. The transfer is coordinated and planned ahead of time by the compiler/DAG. See: [UIs are streaming DAGs (Getz 2022)](https://hyperfiddle.notion.site/UIs-are-streaming-DAGs-e181461681a8452bb9c7a9f10f507991)
...
* (This is new in v3, in v2 site inheritance was lexical, which caused problems.)
